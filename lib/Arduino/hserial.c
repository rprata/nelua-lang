...
void hs_start(const int port, unsigned long baud)
{
	...
	*Serial_[port].ucsrb |= (1 << Serial_[port].rxen);
	*Serial_[port].ucsrb |= (1 << Serial_[port].txen);
	*Serial_[port].ucsrb |= (1 << Serial_[port].rxcie);
...
void hs_writeLine(const int port, const char str[]) {
